...
if (pairingCode && !conn.authState.creds.registered) {
  ...
  if (!!global.pairingNumber) {
    ...
    if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
      console.log(
        chalk.bgBlack(chalk.redBright("Start with your country's WhatsApp code, Example : 92321709972"))
      )
      process.exit(0)
    }
  } else {
    ...
    if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
      console.log(
        chalk.bgBlack(chalk.redBright("Start with your country's WhatsApp code, Example : 923321709972"))
      )
  ...
  setTimeout(async () => {
    ...
    const pairingCode =
      chalk.bold.greenBright('Apka Pairing Code:') + ' ' + chalk.bgGreenBright(chalk.black(code))
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    const msg = `*𝑯𝑨𝑪𝑲𝑬𝑹 ᵀᶠ 🄼🄳
    🄲🄾🄽🄽🄴🄲🅃🄴🄳* \n\n *SUPPORT BY 🅗🅐🅒🅚🅔🅡 🅣🅕 CONTACT NUMBER 923321709972*
*PAWER OF HACKER TF*`
